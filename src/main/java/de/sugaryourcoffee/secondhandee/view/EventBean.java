...
public class EventBean implements Serializable
{
   ...
   public String delete()
   {
      ...
      try
      {
         ...
         Iterator<ItemList> iterItemLists = deletableEntity.getItemLists().iterator();
         for (; iterItemLists.hasNext();)
         {
            ItemList nextInItemLists = iterItemLists.next();
            nextInItemLists.setEvent(null);
            iterItemLists.remove();
            this.entityManager.merge(nextInItemLists);
         }
         Iterator<Selling> iterSellings = deletableEntity.getSellings().iterator();
         for (; iterSellings.hasNext();)
         {
            Selling nextInSellings = iterSellings.next();
            nextInSellings.setEvent(null);
            iterSellings.remove();
            this.entityManager.merge(nextInSellings);
         }
   ...
   private Predicate[] getSearchPredicates(Root<Event> root)
   {
      ...
      int maxLists = this.example.getMaxLists();
      if (maxLists != 0)
      {
         predicatesList.add(builder.equal(root.get("maxLists"), maxLists));
      }
      int maxItemsPerList = this.example.getMaxItemsPerList();
      if (maxItemsPerList != 0)
      {
         predicatesList.add(builder.equal(root.get("maxItemsPerList"), maxItemsPerList));
      }
