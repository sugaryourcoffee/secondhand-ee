...
public class ItemBean implements Serializable
{
   ...
   public String delete()
   {
      ...
      try
      {
         ...
         ItemList itemList = deletableEntity.getItemList();
         itemList.getItems().remove(deletableEntity);
         deletableEntity.setItemList(null);
         this.entityManager.merge(itemList);
         Selling selling = deletableEntity.getSelling();
         selling.getItems().remove(deletableEntity);
         deletableEntity.setSelling(null);
         this.entityManager.merge(selling);
   ...
   private Predicate[] getSearchPredicates(Root<Item> root)
   {
      ...
      int itemNumber = this.example.getItemNumber();
      if (itemNumber != 0)
      {
         predicatesList.add(builder.equal(root.get("itemNumber"), itemNumber));
      }
      ...
      {
         predicatesList.add(builder.like(root.<String> get("size"), '%' + size + '%'));
      }
      ItemList itemList = this.example.getItemList();
      if (itemList != null)
      {
         predicatesList.add(builder.equal(root.get("itemList"), itemList));
      }
      Selling selling = this.example.getSelling();
      if (selling != null)
      {
         predicatesList.add(builder.equal(root.get("selling"), selling));
      }
